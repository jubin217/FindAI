...
const mergeConfigs = (...configs: any[]): any => configs.reduce((acc, c) => ({ ...acc, ...c }), {});
...
const useResponsiveDimension = (responsive: boolean, config: any, key: 'height' | 'width') => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let v = config[key];
      const prefix = w <= 480 ? 'mobile' : w <= 768 ? 'tablet' : w <= 1024 ? 'desktop' : '';
      if (prefix) {
        const responsiveKey = `${prefix}${key[0].toUpperCase() + key.slice(1)}`;
        if (config[responsiveKey] !== undefined) {
          v = config[responsiveKey];
        }
      }
      setValue(v);
    };
